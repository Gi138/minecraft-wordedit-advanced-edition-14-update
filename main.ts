...
function Abuild_Core_Random (randomSlot: number, firstPosition: Position, secondPosition: Position) {
    ...
    for (let y2 = abuildcoreymin; y2 < abuildcoreymin + Math.abs(abuildcoreymax - abuildcoreymin) + 1; y2++) {
        for (let x2 = abuildcorexmin; x2 < abuildcorexmin + Math.abs(abuildcorexmax - abuildcorexmin) + 1; x2++) {
            for (let z2 = abuildcorezmin; z2 < abuildcorezmin + Math.abs(abuildcorezmax - abuildcorezmin) + 1; z2++) {
...
blocks.place(AIR, targercloneblock)
    player.tell(mobs.target(LOCAL_PLAYER), "§5Minecraft WorldEdit Advanced Edition: §eAgent has finished building with random blocks")
}
...
player.onChat("delete", function () {
    ...
    humanReadableOutput = "§5Minecraft WorldEdit Advanced Edition: §eDeleted the selected region"
    player.tell(mobs.target(LOCAL_PLAYER), humanReadableOutput)
})
...
player.onChat("pastedirection", function (direction) {
    builder.teleportTo(player.position())
    if (direction = 0) {
        builder.face(EAST)
    } else if (direction = 1) {
        ...
        player.tell(mobs.target(LOCAL_PLAYER), "§5Minecraft WorldEdit Advanced Edition: §eUnknown Direction")
    }
    if (direction <= 3) {
        builder.paste()
        player.tell(mobs.target(LOCAL_PLAYER), "§5Minecraft WorldEdit Advanced Edition: §ePasted the building in your clipboard to your current position facing the given direction.")
    }
})
...
player.onChat("duplicate", function () {
    builder.copy()
    builder.teleportTo(player.position())
    builder.paste()
    player.tell(mobs.target(LOCAL_PLAYER), "§5Minecraft WorldEdit Advanced Edition: Duplicated the selected region")
})
